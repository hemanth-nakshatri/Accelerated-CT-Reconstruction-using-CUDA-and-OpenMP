...
__global__ void bilinearInterpolateKernel(const unsigned char* image, double* output,
    ...
    for (int s = 0; s < num_samples; ++s) {
        ...
        // Compute the integer coordinates of the four surrounding pixels
        int x1 = static_cast<int>(floor(x));
        int y1 = static_cast<int>(floor(y));
        int x2 = x1 + 1;
        int y2 = y1 + 1;

        // Check if any of the surrounding pixels are out of bounds
    ...
    // Store the result, averaging over the number of samples
...
std::vector<double> CTProcessor::create_sinogram_cuda(const BMPImage& input_image, int block_size) {
    ...
    cudaMalloc(&d_input, input_image.data.size() * sizeof(unsigned char));
    cudaMalloc(&d_grayscale, image_width * image_height * sizeof(unsigned char));
    
    // Copy input data to device
    cudaMemcpy(d_input, input_image.data.data(), 
    ...
    cudaMemcpy(sinogram.data(), d_sinogram,
...
// CUDA kernel to apply ramp filter
__global__ void apply_ramp_filter_kernel(cufftComplex* freq_domain, 
                                         double* ramp_filter, 
                                         int padded_size, 
                                         int num_angles) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (idx < padded_size/2 + 1) {
        // Apply ramp filter to positive frequencies
        int base_idx = angle * padded_size + idx;
        freq_domain[base_idx].x *= ramp_filter[idx];
        freq_domain[base_idx].y *= ramp_filter[idx];
        
        // Apply to symmetric negative frequencies
        if (idx > 0 && idx < padded_size/2) {
            int sym_idx = angle * padded_size + (padded_size - idx);
            freq_domain[sym_idx].x *= ramp_filter[idx];
            freq_domain[sym_idx].y *= ramp_filter[idx];
        }
    }
}


// Apply ramp filter using CUDA
std::vector<double> CTProcessor::apply_ramp_filter_cuda(const std::vector<double>& sinogram, int block_size) {
    ...
    const int num_angles = config.num_angles;
    ...
    double* d_ramp_filter;
    
    cudaMalloc(&d_input, num_angles * padded_size * sizeof(cufftComplex));
    cudaMalloc(&d_output, num_angles * padded_size * sizeof(cufftComplex));
    cudaMalloc(&d_ramp_filter, (padded_size/2 + 1) * sizeof(double));
    
    // Prepare input data on host
    std::vector<cufftComplex> h_input(num_angles * padded_size, {0.0f, 0.0f});
    for (int angle = 0; angle < num_angles; ++angle) {
        for (int i = 0; i < num_detectors; ++i) {
            h_input[angle * padded_size + i].x = sinogram[angle * num_detectors + i];
        }
    }
    
    // Copy input to device
    cudaMemcpy(d_input, h_input.data(), num_angles * padded_size * sizeof(cufftComplex), cudaMemcpyHostToDevice);
    
    // Prepare ramp filter on host
    std::vector<double> h_ramp(padded_size/2 + 1);
    for (int i = 0; i < h_ramp.size(); ++i) {
        h_ramp[i] = i * (2.0 / padded_size);
    }
    
    // Copy ramp filter to device
    cudaMemcpy(d_ramp_filter, h_ramp.data(), (padded_size/2 + 1) * sizeof(double), cudaMemcpyHostToDevice);
    
    // Create cuFFT plan
    cufftHandle plan;
    cufftPlan1d(&plan, padded_size, CUFFT_C2C, num_angles);
    ...
    // Launch kernel to apply ramp filter
    dim3 block(block_size);
    dim3 grid((padded_size + block.x - 1) / block.x, num_angles);
    
    apply_ramp_filter_kernel<<<grid, block>>>(d_output, d_ramp_filter, padded_size, num_angles);
    
    // Synchronize after ramp filter kernel
    cudaDeviceSynchronize();
    
    // Perform inverse FFT
    cufftExecC2C(plan, d_output, d_output, CUFFT_INVERSE);
    
    // Synchronize after inverse FFT
    cudaDeviceSynchronize();
    ...
    // Copy back results
    std::vector<cufftComplex> h_output(num_angles * padded_size);
    cudaMemcpy(h_output.data(), d_output, num_angles * padded_size * sizeof(cufftComplex), cudaMemcpyDeviceToHost);
    
    // Extract real parts
    for (int angle = 0; angle < num_angles; ++angle) {
        for (int i = 0; i < num_detectors; ++i) {
            filtered_sinogram[angle * num_detectors + i] = h_output[angle * padded_size + i].x / padded_size;
        }
    }
...
BMPImage CTProcessor::reconstruct_image_cuda(const std::vector<double>& filtered_sinogram, int threads_per_dim) {
    // std::vector<double> filtered_sinogram = apply_ramp_filter_cuda(sinogram, threads_per_dim * threads_per_dim);
    // std::vector<double> filtered_sinogram = sinogram;
    // Allocate device memory
    float *d_filtered_sinogram, *d_reconstructed;
    cudaMalloc(&d_filtered_sinogram, 
                               filtered_sinogram.size() * sizeof(float));
    cudaMalloc(&d_reconstructed,
    ...
    try{
        ...
        cudaMemcpy(d_filtered_sinogram, float_sinogram.data(),
                                filtered_sinogram.size() * sizeof(float),
                                cudaMemcpyHostToDevice);
        ...
        cudaMemcpy(reconstructed.data(), d_reconstructed,
